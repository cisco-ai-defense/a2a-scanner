...
"""LLM Meta-Analyzer module for A2A Scanner.
...
findings, correlate related issues, and provide actionable recommendations and fixes.
...
Configuration:
    ...
    - A2A_SCANNER_META_LLM_PROVIDER: LLM provider for meta-analyzer
    - A2A_SCANNER_META_LLM_API_KEY: API key for meta-analyzer
    - A2A_SCANNER_META_LLM_MODEL: Model for meta-analyzer
    - A2A_SCANNER_META_LLM_BASE_URL: Base URL for meta-analyzer
    - A2A_SCANNER_META_LLM_API_VERSION: API version for meta-analyzer
    
    If not set, these fall back to the primary LLM settings (A2A_SCANNER_LLM_*).
"""
...
class MetaAnalysisResult:
    ...
    Attributes:
        validated_findings: Findings confirmed as true positives with enriched data.
                          Format matches SecurityFinding.to_dict() for consistency.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary format consistent with ScanResult.to_dict()."""
        return {
    ...
    def get_findings_as_security_findings(self) -> List["SecurityFinding"]:
        """Convert validated findings back to SecurityFinding objects.
        
        Returns:
            List of SecurityFinding objects for consistency with other analyzers.
        """
        findings = []
        for finding_data in self.validated_findings:
            # Build details with meta-analysis enrichments
            ...
            if "confidence_reason" in finding_data:
                details["meta_reason"] = finding_data["confidence_reason"]
            if "exploitability" in finding_data:
                details["exploitability"] = finding_data["exploitability"]
            if "impact" in finding_data:
                details["impact"] = finding_data["impact"]
            if "enriched_details" in finding_data:
                details.update(finding_data["enriched_details"])
            
            finding = SecurityFinding(
                severity=finding_data.get("severity", "UNKNOWN"),
                summary=finding_data.get("summary", ""),
                threat_name=finding_data.get("threat_name", "Unknown"),
...
class LLMMetaAnalyzer:
    ...
    The meta-analyzer runs AFTER all other analyzers complete and can use
    different LLM settings than the primary LLM analyzer.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: Override API key (defaults to config.meta_llm_api_key)
            model: Override model (defaults to config.meta_llm_model)
            base_url: Override base URL (defaults to config.meta_llm_base_url)
            api_version: Override API version (defaults to config.meta_llm_api_version)
        """
        self.name = "LLM_META"
        self.config = config
        ...
        self.api_key = api_key or config.meta_llm_api_key
        self.model = model or config.meta_llm_model
        self.base_url = base_url or config.meta_llm_base_url
        self.api_version = api_version or config.meta_llm_api_version

        if not self.api_key:
            raise ValueError(
                "Meta-Analyzer LLM API key not configured. "
                "Set A2A_SCANNER_META_LLM_API_KEY or A2A_SCANNER_LLM_API_KEY environment variable."
            )
        ...
        if self.model and self.model.startswith("azure/"):
            if not self.base_url:
                raise ValueError(
                    ...
                    "Set A2A_SCANNER_META_LLM_BASE_URL or A2A_SCANNER_LLM_BASE_URL environment variable."
                )
            if not self.api_version:
                raise ValueError(
                    "Azure OpenAI API version not configured for meta-analyzer. "
                    "Set A2A_SCANNER_META_LLM_API_VERSION or A2A_SCANNER_LLM_API_VERSION environment variable."
                )
    ...
    def _load_prompts(self):
        ...
        # Load meta-analysis prompt
        meta_analysis_file = prompts_dir / "meta_analysis_prompt.md"
        if meta_analysis_file.exists():
            self.system_prompt_template = meta_analysis_file.read_text()
        else:
            self.logger.warning(
...
Respond with JSON containing your analysis."""
    ...
    async def _make_llm_request(self, system_prompt: str, user_prompt: str) -> str:
        ...
        api_params = {
            ...
            "messages": messages,
            "temperature": 0.1,
            "max_tokens": 8000,  # More tokens for detailed meta-analysis
    ...
    async def analyze_findings(
        ...
        user_prompt = self._build_user_prompt(
            findings, original_content, context, start_delimiter, end_delimiter
        )
...
The following findings were detected by multiple security analyzers. Please perform meta-analysis to:
...
Please provide your meta-analysis as a JSON object with the structure defined in the system prompt."""
    ...
    def _parse_meta_response(
        ...
        try:
            ...
            return MetaAnalysisResult(
                validated_findings=[f.to_dict() for f in original_findings],
                overall_risk_assessment={
                    "risk_level": "UNKNOWN",
    ...
    def _extract_json_from_response(self, response_content: str) -> Dict[str, Any]:
        ...
        # Handle code blocks anywhere in the response
        try:
            # Look for ```json block
            ...
            if start_idx != -1:
                ...
                if end_idx != -1:
                    ...
                    # No closing ```, try to parse what we have (truncated response)
        ...
        try:
            ...
            if start_idx != -1:
                ...
                if end_idx != -1:
                    ...
                    # Unbalanced braces - try to fix truncated JSON
    ...
    def _enrich_findings(
        ...
        Args:
            ...
            original_findings: Original findings for reference
        """
        # Create index lookup for original findings
        original_lookup = {}
        for i, finding in enumerate(original_findings):
            original_lookup[i] = finding.to_dict() if hasattr(finding, "to_dict") else {
                "severity": finding.severity,
                "summary": finding.summary,
                "threat_name": finding.threat_name,
                "analyzer": finding.analyzer,
    ...
    def get_prioritized_findings(
        ...
        if priority_order:
            # Sort by provided priority order
    ...
    def get_summary_report(self, meta_result: MetaAnalysisResult) -> Dict[str, Any]:
        ...
        return {
            "total_original_findings": len(validated) + len(false_pos),
            "validated_findings_count": len(validated),
            "false_positives_count": len(false_pos),
            "false_positive_rate": (
                len(false_pos) / (len(validated) + len(false_pos)) * 100
                if (validated or false_pos) else 0
            ),
